...
print 'NOTE: this will take a long time to run... generating', total, 'datasets'

# Generate experiments
for j in range(total):
    N = N_dataset_sizes[j]  # Number of observations in this data set
    print 'processing data set {0} (of {1}), for data set of size {2}' \
        .format(j + 1, total, int(N))
    x = np.random.rand(N)
...
plt.plot(N_dataset_sizes, noisevar * (1 - 2.0 / N_dataset_sizes),
         color='r', linewidth=2, label='Theoretical')
